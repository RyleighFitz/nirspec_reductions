...
#Add a flat gaussian function
def flat_gaus(omega_x, omega_y, sigma_y=1, sigma_x=1, A=1):
    return A * np.exp(-((sigma_x * omega_x)**4 + (sigma_y * omega_y)**4) / 2)

def ft_mexh(omega_x, omega_y, sigma_y=1, sigma_x=1, order=2):
    """Mexican hat wavelet as in Wang and Lu (2010), equation [14] altered to have a flat top"""
    return (1. - (omega_x ** 4. + omega_y ** 4.)) * np.exp(-0.5 * (omega_x ** 4. + omega_y ** 4.))

#def ft_mexh(omega_x, omega_y, sigma_y=1, sigma_x=1, order=2):
#    return (1. - (0.5*(omega_x**4. + omega_y**4.))*np.exp(-0.5 * (omega_x**4. + omega_y**4.)))
